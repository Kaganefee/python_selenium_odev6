...
class Test_OdevClass:
    ...
    sheet="Sheet1"
    @pytest.mark.parametrize("username,password",getData(path,sheet))  
    def test_sortby_name_ZtoA(self,username,password):
        self.test_login(username,password)
        sortbtn= Select(self.driver.find_element(By.XPATH,"/html/body/div/div/div/div[1]/div[2]/div/span/select"))
        sortbtn.select_by_value("za")
        self.driver.save_screenshot(f"{self.folderPath}/sort_by_name_ZtoA.png")                    
        
    path="data/standard_user.xlsx"
    sheet="Sheet1"
    @pytest.mark.parametrize("username,password",getData(path,sheet))
    def test_detail(self,username,password):
        self.test_login(username,password)
        product = self.waitForElementVisible((By.ID,"item_2_img_link"))
        product.click()
        self.driver.save_screenshot(f"{self.folderPath}/product_detail.png")
        assert self.waitForElementVisible((By.XPATH,"/html/body/div/div/div/div[2]/div/div/div[2]/div[1]")).text == "Sauce Labs Onesie"
